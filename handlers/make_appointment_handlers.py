...
async def process_email(message: types.Message, state: FSMContext):
    ...
    await message.answer(f'Введіть ваш номер телефону:')
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    request_form["email"] = user_data.get("email")
    request_form["phone"] = user_data.get("phone")

    await fill_booking_data('data', request_form)
    
    status_booking = await create_booking(request_form['name'], request_form['slot'], request_form['firstName'], request_form['email'], request_form['phone'])
