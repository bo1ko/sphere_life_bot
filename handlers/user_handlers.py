...
async def cmd_menu(callback: types.CallbackQuery):
    await callback.message.answer('Ви перейшли в головне меню', reply_markup=kb.main)
...
async def get_service(callback: types.CallbackQuery):
    ...
    name = f'<b>{api_services_data[service_index]["name"]}</b>'
    quote = ''

    if 'Первинна консультація з Лікарем' not in name:
        quote = f'<blockquote><i>{api_services_data[service_index]["tagLine"]}</i></blockquote>'
...
async def get_more_info(callback: types.CallbackQuery):
    service_index = int(callback.data.split('_')[-1])
    name = f'<i><b>{api_services_data[service_index]["name"]}</b></i>'
    quote = ''
